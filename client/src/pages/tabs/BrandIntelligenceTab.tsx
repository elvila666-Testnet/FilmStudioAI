...
export default function BrandIntelligenceTab({ projectId }: BrandIntelligenceTabProps) {
  ...
  const [generatedArchetypes, setGeneratedArchetypes] = useState<any[]>([]);
  const [selectedHeroCharacter, setSelectedHeroCharacter] = useState<any>(null);
  ...
  // TODO: Add characterArchetype router to main appRouter
  // const generateArchetypesMutation = trpc.characterArchetype.generate.useMutation();
  ...
  const handleGenerateArchetypes = async () => {
    if (!selectedBrandId || !getBrandQuery.data) {
    ...
    try {
      ...
      // TODO: Call generateArchetypesMutation once router is added
      // For now, using mock data
      const mockArchetypes = [
        {
          id: "arch-1",
          name: "The Visionary Entrepreneur",
          description: "A forward-thinking leader who embodies innovation",
          personality: "Ambitious, creative, confident",
          targetAudience: getBrandQuery.data.targetCustomer || "",
          archetypeType: "Leader",
          visualDescription: "Modern professional with cutting-edge style",
          brandAlignment: 95,
          reasoning: "Perfectly aligns with brand mission and aesthetic",
          variants: [
            {
              id: "var-1",
              imageUrl: "https://via.placeholder.com/300x400?text=Character+1",
              description: "Professional attire variant",
      ...
      setGeneratedArchetypes(mockArchetypes);
    } catch (error) {
      console.error("Failed to generate archetypes:", error);
      alert("Failed to generate character archetypes. Please try again.");
    } finally {
      setIsGeneratingArchetypes(false);
    }
  };

  const handleSelectHero = (archetype: any, variantId: string) => {
    setSelectedHeroCharacter({ archetype, variantId });
  };
  ...
  return (
    <div className="space-y-6">
      ...
      {generatedArchetypes.length > 0 && (
        <Card className="bg-card border-border border-accent">
          <CardHeader>
            <CardTitle className="text-lg">Character Archetypes</CardTitle>
            <CardDescription>
              Select a hero character to represent your brand
            </CardDescription>
          </CardHeader>
          <CardContent>
            <CharacterArchetypeSelector
              archetypes={generatedArchetypes}
              loading={isGeneratingArchetypes}
              onSelectHero={handleSelectHero}
              onRetake={handleGenerateArchetypes}
            />
          </CardContent>
        </Card>
      )}
